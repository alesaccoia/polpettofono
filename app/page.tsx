...
type AudioRig = { context: AudioContext; node: AudioWorkletNode; gain: GainNode };
...
const zones: SoundZone[] = [
  ...
  { id: 'chiamata', part: 'occhi', name: 'Chiamata', color: '#54c7b7', x: 48, y: 29, radius: 14, file: '/audio/cat-call.ogg' },
  { id: 'gorgheggio', part: 'zampa', name: 'Gorgheggio', color: '#9776f2', x: 23, y: 68, radius: 24, file: '/audio/chirp.wav' },
];
...
class PhaseMorphProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = ({ data }) => { if (data.type === 'sources') this.sources = data.sources; if (data.type === 'trigger') { this.weights = data.weights; this.trigger(Boolean(data.harmony)); } };
  }
  trigger(harmony) { this.ring.fill(0); this.read = 0; this.available = 0; this.framesUntilNext = 0; const pitches = harmony ? [1, 1.2599, 1.4983] : [1]; const gains = harmony ? [0.72, 0.22, 0.16] : [0.92]; this.voices = pitches.map((pitch, index) => ({ position: 0, remaining: Math.floor(this.sampleRateValue * 1.38), pitch, gain: gains[index] })); }
  ...
  makeFrame() {
    ...
    for (const voice of this.voices) {
      const reals = [], imags = [];
      for (const source of this.sources) { const real = new Float32Array(this.N), imag = new Float32Array(this.N); for (let i = 0; i < this.N; i++) { const index = Math.floor(voice.position + i * voice.pitch); real[i] = index < source.length ? source[index] * this.window[i] : 0; } this.fft(real, imag, false); reals.push(real); imags.push(imag); }
      ...
      const attack = Math.min(1, voice.position / (this.sampleRateValue * 0.018)); const release = Math.min(1, voice.remaining / (this.sampleRateValue * 0.16));
      for (let i = 0; i < this.N; i++) this.ring[(start + i) % this.ring.length] += outReal[i] * this.window[i] * attack * release * voice.gain * 1.75;
      voice.position += this.H * voice.pitch; voice.remaining -= this.H;
    }
    this.voices = this.voices.filter((voice) => voice.remaining > 0); this.available += this.H;
  }
...
export default function Home() {
  ...
  const [isMuted, setIsMuted] = useState(false); const [sequence, setSequence] = useState<Array<number | null>>([0, null, 1, null, 2, null, 3, null]); const [isSequencing, setIsSequencing] = useState(false); const [sequencerStep, setSequencerStep] = useState(-1);
  const [harmonyOn, setHarmonyOn] = useState(true); const [rhythmOn, setRhythmOn] = useState(false); const audioRef = useRef<AudioRig | null>(null); const sequenceRef = useRef(sequence); const rhythmTimerRef = useRef<number | null>(null);
  ...
  const startAudio = useCallback(async () => {
    ...
    const buffers = await Promise.all(zones.map(async (zone) => context.decodeAudioData(await (await fetch(zone.file)).arrayBuffer()))); const node = new AudioWorkletNode(context, 'phase-morph-processor'); const gain = context.createGain(); gain.gain.value = 0.72; node.connect(gain).connect(context.destination);
    const sources = buffers.map((buffer) => buffer.getChannelData(0).slice()); node.port.postMessage({ type: 'sources', sources }, sources.map((source) => source.buffer)); audioRef.current = { context, node, gain }; return audioRef.current;
  }, []);
  const triggerWeights = useCallback(async (nextWeights: number[]) => { const audio = await startAudio(); audio.node.port.postMessage({ type: 'trigger', weights: nextWeights, harmony: harmonyOn }); }, [harmonyOn, startAudio]);
  ...
  useEffect(() => { if (audioRef.current) audioRef.current.gain.gain.value = isMuted ? 0 : 0.72; }, [isMuted]); useEffect(() => () => { audioRef.current?.context.close(); }, []);
  ...
  return (
    <main className="site-shell">
      ...
      <section className="photo-stage"><div className="cat-photo-wrap" onPointerDown={handlePointer} role="button" tabIndex={0} aria-label="Tocca la foto del gatto" onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') void playPoint(active); }}><img src="/images/cat.jpg" alt="Il gatto del Cattofono" className="cat-photo" draggable={false} /><div className="photo-shade" /><div className="crosshair" style={{ left: `${active.x}%`, top: `${active.y}%` }}><span /><span /></div>{zones.map((zone) => <span key={zone.id} className={`epicenter ${activeZone.id === zone.id ? 'selected' : ''}`} style={{ left: `${zone.x}%`, top: `${zone.y}%`, '--zone-color': zone.color } as React.CSSProperties}><i /></span>)}</div></section>
      <section className="sequencer-section" aria-label="Sequencer"><div className="seq-panel"><div className="seq-grid">{sequence.map((zoneIndex, index) => <button key={index} className={`seq-step ${sequencerStep === index ? 'playing' : ''} ${zoneIndex === null ? 'empty' : ''}`} onClick={() => cycleStep(index)} aria-label={`Step ${index + 1}: ${zoneIndex === null ? 'vuoto' : zones[zoneIndex].name}`} style={zoneIndex === null ? undefined : { '--step-color': zones[zoneIndex].color } as React.CSSProperties}>{zoneIndex !== null && <i />}</button>)}</div><div className="seq-controls"><button className={`icon-button play ${isSequencing ? 'active' : ''}`} onClick={() => void toggleSequencer()} aria-label={isSequencing ? 'Ferma sequencer' : 'Avvia sequencer'}>{isSequencing ? '■' : '▶'}</button><button className={`icon-button ${harmonyOn ? 'active' : ''}`} onClick={() => setHarmonyOn((value) => !value)} aria-label={harmonyOn ? 'Disattiva armonie' : 'Attiva armonie'} aria-pressed={harmonyOn}>♫</button><button className={`icon-button ${rhythmOn ? 'active' : ''}`} onClick={() => void toggleRhythm()} aria-label={rhythmOn ? 'Disattiva base ritmica' : 'Attiva base ritmica'} aria-pressed={rhythmOn}>♩</button></div></div></section>
    </main>
  );
}
